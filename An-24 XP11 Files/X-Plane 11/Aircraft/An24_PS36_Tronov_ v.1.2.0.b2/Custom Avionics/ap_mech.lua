...
function update()
	...
	local roll_spd_now = get(roll_spd) * 2 -- current roll speed
	...
	local ail_step = math.min(0.5, math.abs(roll_spd_now - roll_spd_need)*0.008)
	...
	local elev_step = math.min(0.5, math.abs(pitch_spd_now - pitch_spd_need) * 0.05)
	...
	local rudd_step = math.min(0.1, math.abs(hdg_spd_now - hdg_spd_need) * 0.015)
