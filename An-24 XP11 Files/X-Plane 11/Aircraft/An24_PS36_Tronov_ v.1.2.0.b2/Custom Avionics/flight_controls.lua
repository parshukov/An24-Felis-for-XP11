...
function update()
	-- initialise
	    ...
	    -- set(overr, 1) -- заменяем на альтернативу
        -- get(left_eng_main) == 1 and get(left_eng_ext) == 1 and get(right_eng_main) == 1 and get(right_eng_ext) == 1 and get(ru19_eng_ext) == 1 and 
        
        if get(antiice_left) == 1 and get(antiice_right) == 1 and get(rockets) == 1 and get(pitot_1) == 1 and get(pitot_2) == 1 and get(pitot_3) == 1 then -- если заглушки и крышки сняты, 
        set(overr, 1)	-- оверрайд есть
        else
        set(overr, 0)	-- оверрайд убран	
        end	
