...
local plastic_button = loadSample('Custom Sounds/plastic_btn.wav')
local ground_power_request = loadSample('Custom Sounds/ground_power_request.wav')
local ground_power_remove = loadSample('Custom Sounds/ground_power_remove.wav')
...
function update()
	...
	elseif pax_ladder_status > 0.95 then pax_ladder_img = get(empty)
        ...
        -- controls realised        
                if get(left_eng_main) == 0 and get(left_eng_ext) == 0 and get(right_eng_main) == 0 and	get(right_eng_ext) == 0 and	get(ru19_eng_ext) == 0 then
                set(override, 0)
              --  print("throtles released")  
                set(override_1, 0)
	            set(override_2, 0)  end
...
--		position = { 240, 428, 30, 30 },
    ...
    switch {
		...
		playSample(plastic_button, 0)
				...
				playSample(ground_power_request, 0)
			    set(ground_available, 4)
				end
				
				if get(ground_available) == 1 then
				playSample(ground_power_remove, 0)
