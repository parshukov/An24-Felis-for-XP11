...
--	print(power)
	...
	if power_counter > 0 and power_counter < 1 then
		if not isSamplePlaying(ark_start_sound) then playSample(ark_start_sound, 1 ) end 
	end
...
if get(head_z)>-5.625 or get(head_z)<-7.6  then local_cockpit_z=0 else  local_cockpit_z=1 end  --Пример 
...
------------------------	
	if cockpits==0 then stopSample(ark_start_sound) end
