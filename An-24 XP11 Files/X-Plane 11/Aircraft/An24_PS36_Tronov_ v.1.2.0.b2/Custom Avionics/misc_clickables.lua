...
defineProperty("indicated_airspeed", globalPropertyi("sim/flightmodel/position/indicated_airspeed")) -- приборная скорость
...
local window_open_sound = loadSample('Custom Sounds/w_open.wav')
local window_close_sound = loadSample('Custom Sounds/w_close.wav')

local cabin_chair_sound = loadSample('Custom Sounds/cabin_chair.wav')

local door_open_sound = loadSample('Custom Sounds/door_large_open.wav')
local door_close_sound = loadSample('Custom Sounds/door_large_close.wav')
    ...
    switch {
        ...
        onMouseClick = function()
            if get(cabin_left_window) ~= 0 then
                playSample(window_close_sound, 0)
                set(cabin_left_window, 0)
            else
                playSample(window_open_sound, 0)
    ...
    switch {
        ...
        onMouseClick = function()
            if get(cabin_chair) ~= 0 then
                ...
                playSample(cabin_chair_sound, 0) 
                set(cabin_chair, 0)
            else
                playSample(switch_sound, 0)
                playSample(cabin_chair_sound, 0)
