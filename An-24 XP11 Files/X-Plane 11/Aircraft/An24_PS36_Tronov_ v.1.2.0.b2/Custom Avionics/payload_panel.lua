...
-- define variables
local empty_plane_weight = 14180 --14150 -- kg
...
local flight_distance = 0 -- 300 -- km
local reserv_distance = 0 -- 100 -- km
local taxi_time = 0 -- 20 -- min
local fuel_stock = 0 -- 100 -- kg

local crew_weight = 0 --160 -- kg
local stuard_weight = 0 -- 120 -- kg
...
local row_1_mass = 0 --150
local row_2_mass = 0 --150
local row_3_mass = 0 --150
local row_4_mass = 0 --150
local row_5_mass = 0 --150
local row_6_mass = 0 --150
...
local pos1 = 237 -- initial position in pixels было 237. 
local pos2 = 237 + pos1
local pos3 = 237 + pos2
local pos4 = 237 + pos3
local pos5 = 237 + pos4
...
local to_gear_CG = 22  -- 33
local lan_gear_CG = 22  -- 33

local tank1_load = 0 --125
local tank2_load = 0 --550
local tank3_load = 0 --550
local tank4_load = 0 --125

local acf_CG_meters = 0 --0.102722
...
function set_flags()
	...
	to_cg_flag = to_CG < 14.9 or to_CG > 33
	lan_cg_flag = lan_CG < 14.9 or lan_CG > 33
	to_gear_cg_flag = to_gear_CG < 14.9 or to_gear_CG > 33
	lan_gear_cg_flag = lan_gear_CG < 14.9 or lan_gear_CG > 33	
end
...
function update()
    ...
    aeroplane_weight = loaded_plane_weight + all_pax_mass + cargo_1 + cargo_2 + cargo_3 - fuel_weight
	comercial_payload = all_pax_mass + cargo_1 + cargo_2 + cargo_3
	landing_weight = airplane_weight - used_fuel
	
	-- calculate CG position
	pos1 = 237 - crew_weight * 0.145625 -- изначально, было 237
	...
	local to_coef = 1.5918 * (aeroplane_weight - 14000) * 0.008125 / 65
	local to_CG_low = 33 - (332 - pos11) * 0.1235
	to_CG = to_CG_low + to_coef *  (33 - to_CG_low) * 0.29
	
	local lan_coef = 1.5918 * (aeroplane_weight - 14000) * 0.008125 / 65
	local lan_CG_low = 33 - (332 - pos11) * 0.1235
	lan_CG = lan_CG_low + lan_coef *  (33 - lan_CG_low) * 0.29
	
	to_gear_CG = to_CG - interpolate(gear_tbl, aeroplane_weight)
	...
	local set_coef = 1.5918 * (aeroplane_weight - 14000) * 0.008125 / 65 ---/- fuel_weight Зачем это, уже вычтено топливо.
	local set_CG_low = 33 - (332 - pos11) * 0.1235
	set_CG = set_CG_low + set_coef * (33 - set_CG_low) * 0.29	

	acf_CG_meters = (set_CG - 22.0) * 0.02686
    ...
    clickable {
       ...
       cursor = { 
        ...
        onMouseClick = function()
			...
			if crew_weight < 80 then crew_weight = 0 end --80 
    ...
    clickable {
       ...
       cursor = { 
        ...
        onMouseClick = function()
			...
			if stuard_weight < 80 then stuard_weight = 0 end  -- 80 
        return true
        end
    },	
